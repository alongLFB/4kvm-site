...
export default function RoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Header Bar */}
      {/* Room Header Info */}
      <div className="p-3.5 sm:p-5 rounded-2xl bg-dark-900 border border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 shadow-xl">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <button
            onClick={handleBackClick}
            className="p-2 rounded-xl bg-dark-800 hover:bg-dark-700 active:scale-95 text-gray-300 hover:text-white transition shrink-0"
            title="退出放映厅"
          >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <h1 className="text-sm sm:text-lg font-bold text-white truncate">{room.title}</h1>
              <span className="px-1.5 py-0.5 text-[10px] font-bold text-cyan-400 bg-cyan-500/10 border border-cyan-500/30 rounded-md shrink-0">
                #{room.id}
              </span>
              {!room.isPublic && (
                <span className="px-1.5 py-0.5 text-[10px] font-bold text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded flex items-center gap-0.5 shrink-0">
            ...
            <p className="text-[11px] sm:text-xs text-gray-400 flex items-center gap-1.5 mt-0.5 truncate">
              ...
              <span>·</span>
              <span className="text-cyan-400 font-semibold shrink-0">{room.episodeName}</span>
              <span>·</span>
        ...
        <div className="flex items-center justify-end gap-1.5 sm:gap-2.5 flex-wrap">
          ...
          <button
            ...
            className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-gradient-to-r from-blue-600/20 to-cyan-500/20 hover:from-blue-600 hover:to-cyan-500 active:scale-95 text-cyan-300 hover:text-dark-950 font-bold text-xs transition flex items-center gap-1 sm:gap-1.5 border border-cyan-500/30 shadow-md"
          >
            <Film className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">更换放映影片</span>
          ...
          {isHost && (
            <button
              ...
              className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-gold-400/15 hover:bg-gold-400 active:scale-95 text-gold-400 hover:text-dark-950 font-bold text-xs transition flex items-center gap-1 sm:gap-1.5 border border-gold-400/30 shadow-md"
            >
              <Settings className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">房间设置</span>
              <span className="sm:hidden">设置</span>
            </button>
          )}

          {/* Guest badge */}
          <div className="flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 rounded-xl bg-dark-800 border border-white/10 text-xs">
            ...
            {editingName ? (
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="w-16 sm:w-24 bg-dark-950 text-base sm:text-xs text-white px-1.5 py-0.5 rounded border border-cyan-500 focus:outline-none"
                />
              ...
              <div className="flex items-center gap-1">
                <span className="font-bold text-white max-w-[60px] sm:max-w-none truncate">{currentUser.name}</span>
                <button onClick={() => setEditingName(true)} className="text-gray-500 hover:text-cyan-400 p-0.5">
          ...
          <button
            ...
            className="px-2.5 sm:px-3.5 py-1.5 rounded-xl bg-cyan-500/10 hover:bg-cyan-500 active:scale-95 text-cyan-400 hover:text-dark-950 font-bold text-xs transition flex items-center gap-1 sm:gap-1.5 border border-cyan-500/30 shadow-md"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 space-y-3 sm:space-y-4 relative group">
          {/* Floating Speaker Overlay Badge */}
          {otherSpeaking.length > 0 && (
            <div className="absolute top-2.5 left-2.5 sm:top-4 sm:left-4 z-40 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-black/85 border border-emerald-500/40 text-emerald-300 text-[11px] sm:text-xs font-bold backdrop-blur-md flex items-center gap-1.5 sm:gap-2 shadow-2xl animate-in fade-in slide-in-from-top-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]" />
              <span>{otherSpeaking[0].avatar}</span>
              <span className="truncate max-w-[140px] sm:max-w-none">{otherSpeaking.map((s) => s.name).join("、")} 正在说话...</span>
            </div>
          )}
          <RoomVideoPlayer
            url={room.streamUrl}
          ...
          {/* Quick Info bar */}
          <div className="p-3 sm:p-4 rounded-2xl bg-dark-900 border border-white/10 flex items-center justify-between gap-2 text-[11px] sm:text-xs text-gray-400">
            <div className="flex items-center gap-1.5 sm:gap-2 truncate">
              <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)] shrink-0" />
              <span className="truncate">当前线路：<strong className="text-cyan-400">{currentSource.sourceName}</strong> · 延迟 &lt; 30ms</span>
            </div>
        ...
        <div className="bg-dark-900 border border-white/10 rounded-2xl flex flex-col h-[460px] sm:h-[520px] lg:h-[580px] shadow-2xl overflow-hidden">
          {/* Tabs */}
