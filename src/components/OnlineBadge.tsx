...
export function OnlineBadge({ className = "", showIcon = true }: OnlineBadgeProps) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-xs font-medium text-cyan-300 select-none shadow-sm transition-all hover:bg-cyan-500/15 ${className}`}
      title="4KVM 全站实时在线观影人数 (每25秒自动同步)"
    >
      {/* Static Green Dot */}
      <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />

      {showIcon && <Users className="w-3 h-3 text-cyan-400 hidden sm:inline" />}
      ...
      <span className="text-[11px] text-cyan-400/90 sm:hidden">
        人在线
      </span>
    </div>
  );
